<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            {{__('translate.Edit Branch')}} - {{ $branch->name }}
        </h2>
    </x-slot>
            ...
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <fieldset class="p-5 border rounded-xl">
                        <legend class="p-2 text-lg font-bold">{{__('translate.Edit Branch')}} </legend>
                        @if ($errors->any())
                            <div>
                                <ul>
                        ...
                        <form method="POST" action="{{ route('branches.update', $branch->id) }}">
                            ...
                            <div class="grid w-full grid-cols-2 gap-4">
                                ...
                                <!-- ... -->
                                <div class="w-full">
                                    <x-input-label>{{__('translate.location')}}</x-input-label>
                                ...
                                <div class="w-full">
                                    <x-input-label>{{__('translate.company')}}</x-input-label>
                                    <select name="company_id">
                                        <option disabled selected value="">{{__('translate.Select Company')}}</option>
